...
// 增加对是否当前焦点的查询功能，add by iFire
bool GfxEdit::IsFocus()
{
    return g_lpFocusEditPtr == this;
}
...
void GfxEdit::OnChar( UINT nChar, UINT nRepCnt, UINT nFlags )
{
	...
	{}
	else if (m_pHGE->Input_GetKeyState(HGEK_SHIFT)
         && (m_pHGE->Input_GetKeyState(HGEK_RIGHT) || m_pHGE->Input_GetKeyState(HGEK_LEFT)))
    {}
    else
	{
		MouseLDown=false;
...
bool GfxEdit::DoKeyDown()
{
	...
	{
		for (int i=0;i<4;++i) InsertChar(L' ');	// insert 4 space
		return	true;
	}
	...
	else if (m_pHGE->Input_GetKeyState(HGEK_DELETE)) // key delete
	{
        if (MouseL1CharPos != MouseL2CharPos)
            DeleteMoreChar();                       // 令delete时也可以删除一整段文字，add by iFire
		else 
            DeleteChar(DP_RIGHT);
		return	true;
	}
	...
	else if (m_pHGE->Input_GetKeyState(HGEK_LEFT)) // key left
	{
        ...
        if (m_pHGE->Input_GetKeyState(HGEK_SHIFT))  // 添加按下shift时可以选择文字功能，add by iFire
            OnShiftLeftOrRight();
	...
	{
        memset(m_sCookie, 0, sizeof(m_sCookie));    // 按ESC时，清空所有文字，add by iFire
        m_nCharPos = 0;
		return	true;
	}
    else if (m_pHGE->Input_GetKeyState(HGEK_ENTER)) // 按ENTER时，文字不应换行，add by iFire
    {
        return	true;
    }
...
// 添加按下shift时可以选择文字功能，add by iFire
void GfxEdit::OnShiftLeftOrRight()
{
    MouseL2CharPos = m_nCharPos;
    wchar_t sTmp2[COOKIE_SIZE];
    memset(sTmp2,0,sizeof(sTmp2));
    wcsncpy_s(sTmp2,m_sCookie,MouseL2CharPos);
    SIZE szTmp2 = m_pFont->GetTextSize(sTmp2);
    ...
    wchar_t sTmp1[COOKIE_SIZE];
    memset(sTmp1,0,sizeof(sTmp1));
    wcsncpy_s(sTmp1, m_sCookie, MouseL1CharPos);
    SIZE szTmp1 = m_pFont->GetTextSize(sTmp1);
    float tmp_w1 = static_cast<float>(szTmp1.cx);

    MouseL1x = text_x + tmp_w1;
    MouseL2x = text_x + tmp_w2 + 1 - 2;	//补正坐标（请参考GfxEdit::Render(float x, float y)）
    MouseL1y = box_y;
    MouseL2y = box_y + box_h;

    if (MouseL1x < MouseL2x)
    {
        float temp = MouseL1x;
        MouseL1x = MouseL2x;
        MouseL2x = temp;
