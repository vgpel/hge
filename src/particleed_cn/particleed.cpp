...
PEditorState	state;
hgeParticleSystemInfo syspar[9];
...
void InitEditor()
{
	...
	hge->Resource_AttachPack("particleed.dat");
	...
	{
		size = 0;
		sprintf(buf,"particle%d.psi",i+1);
		void* pbuffer = hge->Resource_Load(buf,&size);
		if (size == sizeof(hgeParticleSystemInfo))
			memcpy(&syspar[i],pbuffer,(size_t)size);
	}
...
void CreateGUI()
{
	...
	gui->AddCtrl(new hgeGUIButton(CMD_LOAD, 641, 474, 73, 17, texGui, 336, 191));
	gui->AddCtrl(new hgeGUIButton(CMD_SAVE, 718, 474, 73, 17, texGui, 336, 191));
	gui->AddCtrl(new hgeGUIButton(CMD_EXIT, 718, 494, 73, 17, texGui, 336, 191));
	button=new hgeGUIButton(CMD_HELP, 641, 494, 73, 17, texGui, 336, 191);
	button->SetMode(true);
	gui->AddCtrl(button);
