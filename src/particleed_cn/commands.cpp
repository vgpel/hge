...
{
	state.ps->Stop();
	if(state.nPreset != -1)	ButtonSetState(CMD_PRESET1+state.nPreset, false);
	ButtonSetState(CMD_PRESET1+n, true);
	/*cmdSavePreset(state.nPreset);*/			//改变系统指定粒子时不在默认保存改变的状态
	cmdLoadPreset(n);
	state.nPreset=n;
}
...
void cmdSaveAs()
{
	...
	ofn.hwndOwner = hWnd;
	ofn.lpstrFilter = "粒子配置文件(*.psi)\0*.psi\0所有文件(*.*)\0*.*\0";
	ofn.nFilterIndex = 1;
	sprintf(buf,"save_demo.psi");
	ofn.lpstrFile = buf;
	ofn.nMaxFile = MAX_PATH;
	ofn.lpstrTitle = "请设置存盘文件名：";
	ofn.lpstrInitialDir = ".";
	...
	FILE *pf = fopen(ofn.lpstrFile,"wb");
	if(!pf)
	{
		MessageBox(NULL,"无法保存粒子配置文件!","错误",MB_OK);
		return;
	}

	state.ps->info.sprite=(hgeSprite*)(sprParticles->GetFrame() | sprParticles->GetBlendMode()<<16);

	fwrite(&state.ps->info,1,sizeof(hgeParticleSystemInfo),pf);

	fclose(pf);

	state.ps->info.sprite=sprParticles;

	cmdLoadAs(ofn.lpstrFile);
}
void cmdLoadAs(char* filepath)
{
	DWORD col1, col2;
	HWND hWnd = hge->System_GetState(HGE_HWND);
	char buf[1024];
	...
	if(!pf)
	{
		MessageBox(NULL,"无法打开粒子配置文件!","错误",MB_OK);
		return;
	}

	size_t size = sizeof(hgeParticleSystemInfo);
	hgeParticleSystemInfo temp;

	if(fread(&temp,1,size,pf) != size)
	{
		MessageBox(NULL,"无法打开粒子配置文件!","错误",MB_OK);
	}
	else
